...
class _ProductListState extends State<ProductList> {
  ...
  void initState() {
    super.initState(); // super.initState() 호출 추가
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // (3)만약에 이미지가 존재하면 대표 이미지 1개 추출 없으면 기본이미지 추출
        String imageUrl;
        if(images.isEmpty) { // 리스트가(이미지들) 가 비어있으면
          imageUrl = "$baseUrl/upload/default.jpg";
        } else { // 비어있지 않으면 첫번째 이미지만 추출
          imageUrl = "$baseUrl/upload/${images[0]}";
        }
        // (4) 위젯
        return InkWell( // 해당 위젯을 클릭(탭:모바일 터치) 하면 상세 페이지로 이동 구현
          onTap: () {}, // 만약에 하위 위젯을 클릭 했을 때 이벤트 발생
          child: Card(
            margin: EdgeInsets.all(15), // 여백넣기
            child: Padding(

...
class _UpdateSate extends State<Update> { // 클래스명 앞에 _ 언더바는 dart에서 private 키워드
  ...
  void todoFindById( int id ) async {
    try{
      final response = await dio.get("https://then-heloise-itdanjalog-5d2c7fb5.koyeb.app/");
  ...
  void todoUpdate(  ) async{
    try{
      ...
      final response =  await dio.put("https://then-heloise-itdanjalog-5d2c7fb5.koyeb.app/" , data : sendData );
      final data = response.data;
